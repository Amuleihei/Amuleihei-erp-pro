...
public class FlowImgService {
     ...
     * @return
     */
    public byte[] generateImageByProcInstId(String procInstId) {
        if (StringUtils.isEmpty(procInstId)) {
            LOG.error("[错误]-传入的参数procInstId为空！");
            throw new CustomException("[异常]-传入的参数procInstId为空！");
        }
        ...
        try {
            ...
            throw new CustomException("通过流程实例ID" + procInstId + "获取流程图时出现异常！", e);
        } finally {
